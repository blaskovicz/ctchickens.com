...
export default createStore({
  ...
  actions: {
    async initAuth({ commit, dispatch, getters }: ActionContext<State, State>) {
      ...
      try {
        // 1. Resolve any incoming redirect (this works even if main.ts already started it)
        const result = await getRedirectResult(auth);
        if (result?.user) {
          console.log("Auth: Redirect user found, sync processing...");
          const user = result.user;
          const fbProfile = user.providerData?.find(p => p.providerId === 'facebook.com');

          await setDoc(doc(db, 'users', user.uid), {
            displayName: user.displayName,
            email: user.email,
            photoURL: user.photoURL,
            facebookUid: fbProfile?.uid || null,
      ...
      // 2. Wait for current session (if any) to settle
      await new Promise<void>((resolve) => {
        onAuthStateChanged(auth, async (user) => {
          console.log("Auth: State ->", user ? "User connected" : "Guest");
